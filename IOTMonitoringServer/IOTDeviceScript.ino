...
//Definicion pines para comunicacion SPI con pantalla OLED
#define OLED_MOSI   D7
#define OLED_CLK	  D1
#define OLED_DC		  D5
#define OLED_CS		  D8
...
const char ssid[] = "OppoReno7"; // TODO cambiar por el nombre de la red WiFi
// Contraseña de la red WiFi
const char pass[] = "alejandro"; // TODO cambiar por la contraseña de la red WiFi
...
const char MQTT_HOST[] = "54.86.138.130"; // TODO Reemplace ip.maquina.mqtt por la IP del bróker MQTT que usted desplegó. Ej: 192.168.0.1
...
float humi;
//Valor del calculo del indice de temperatura
float indice_temp;
...
void configureMQTT() {
  ...
  Serial.println("Ingreso a configure");
  client.setServer(MQTT_HOST, MQTT_PORT);
  Serial.println("Ingreso a client.setCallback(receivedCallback);");
  // Se configura la función que se ejecutará cuando lleguen mensajes a la suscripción
  client.setCallback(receivedCallback);
  Serial.println("llamado mqtt_connect");
...
void measure() {
  if ((millis() - measureTime) >= MEASURE_INTERVAL * 1000 ) {
    ...
    if (checkMeasures(temp, humi)) {
      //Se agrega la medida de indice de calor computeHeatIndex en grados celsius
      indice_temp = readIndiceCalor(temp, humi);
      // Se envían los datos
      sendSensorData(indice_temp, humi);
    }
  }
}

/**
   Lee la el indice de calor del sensor DHT, la imprime en consola y la devuelve.
*/
float readIndiceCalor(float t, float h) {

  // Se lee el indice de calor en grados centígrados (por defecto)
