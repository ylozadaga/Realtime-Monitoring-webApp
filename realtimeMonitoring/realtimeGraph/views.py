...
def get_dewpoint(request, **kwargs):
    ...
    for location in locations:
        ...
        location_temp_data = (Data.objects.filter(
            station__in=stations, measurement__name=temperature.name, time__gte=start_ts, time__lte=end_ts)
                              .order_by('time'))

        location_hum_data = (Data.objects.filter(
            station__in=stations, measurement__name=humidity.name, time__gte=start_ts, time__lte=end_ts)
                              .order_by('time'))
...
def get_dewpoint_data(location_temp_data, location_hum_data):
    ...
    for (temp, hum) in zip(location_temp_data, location_hum_data):
        dewpoint = pow((hum.avg_value/100), 0.125) * (112 + (0.9 * temp.avg_value)) + (0.1 * temp.avg_value) - 112
...
def get_formatted_temp_data(location_temp_data, location, stations, data):
    min_val = location_temp_data.aggregate(Min("min_value"))["min_value__min"]
    max_val = location_temp_data.aggregate(Max("max_value"))["max_value__max"]
    avg_val = location_temp_data.aggregate(Avg("avg_value"))["avg_value__avg"]
